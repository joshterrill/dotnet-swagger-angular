...
export class SampleDataClient {
    ...
    generate(): Observable<FileResponse> {
        let url_ = this.baseUrl + "/api/SampleData/generate";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGenerate(response: HttpResponseBase): Observable<FileResponse> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
